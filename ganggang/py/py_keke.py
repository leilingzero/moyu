    ...
    def fetch_image_domain(self):
        ...
        try:
            ...
            for script_src in script_tags:
                if 'rdul.js' in script_src:
                    img_script_url = f"{self.home_url}{script_src}" if script_src.startswith('/') else script_src
                    break
            
            if not img_script_url:
                raise RuntimeError('未能找到rdul.js脚本')
                
            # 请求脚本内容
            script_res = requests.get(img_script_url, headers=self.headers)
            script_content = script_res.text
            
            # 提取图片域名
            match = re.search(r"'(.*?)'", script_content)
            if match and match.group(1):
                image_domain = match.group(1)
                print(f'成功获取图片域名: {image_domain}')
                return image_domain
            else:
                raise RuntimeError('未能从脚本中提取图片域名')
                
        except Exception as e:
            print(f'获取图片域名时出错: {str(e)}')
            return None
                ...
                d.append(
                    {
                        ...
                        'vod_pic': f'{self.image_domain}{i.xpath(".//img/@data-original")[1]}',
                ...
                d.append(
                    {
                        ...
                        'vod_pic': f'{self.image_domain}{i.xpath(".//img/@data-original")[0]}',
